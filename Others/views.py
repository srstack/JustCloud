...
def systemMain(request, username, sid):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            # 判断是否有管理权限
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Others':
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Others':
                ...
                return render(request, 'other/otherAnaly.html', locals())
            else:
                return redirect('/admin/' + request.session.get('USERNAME'))
        else:
            return redirect('/admin/' + request.session.get('USERNAME'))
...
def deviceDetail(request, username, sid, did):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            if user_obj in device_obj.system.admin.all() and system_obj.platform == 'Others':
