...
def systemMain(request, username, sid):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            # 判断是否有管理权限
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Lumiere':
            ...
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Lumiere':
                ...
                for device in devices:
                    if Data.objects.filter(model=0, device=device) and device not in system_waring_devices:
                        for data in Data.objects.filter(~Q(waring=0) & ~Q(waring=1), model=0, device=device,
                                                        date__gte=time_list[0]):
                            for i in range(0, 30):
                                if data.date.date() == time_list[i]:
...
def deviceDetail(request, username, sid, did):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            if user_obj in device_obj.system.admin.all() and system_obj.platform == 'Lumiere':
        ...
        if is_login:
            if request.session.get('USERNAME') == username:
                ...
                if user_obj in device_obj.system.admin.all() and system_obj.platform == 'Lumiere':
                    ...
                    elif device_map['Bottom-Light'] == 1:
                        new_map_data['status'] = 'light'
                    else:
