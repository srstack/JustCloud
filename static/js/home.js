...
function changeDoman() {
    ...
    if ($('.baseinfo .grid .baseinfo-user >span').html() === '管理员用户') {
        $('#domainchange').css('display', 'block');
    }
    else {
...
function domainchange() {
    ...
    $.each(reg_value, function (i, val) {
        if (reg_value[i].value === "") {
            $('#domainchange .change-table .change-table-form > div > form #' + reg_value[i].name).addClass('waring');
            Status = false;
        }
    });
    if (Status === true) {
...
function useradd() {
    var Status = true;
    var reg_value = $('#usertable .change-table .change-table-form div form').serializeArray();
    $.each(reg_value, function (i, val) {
        if (reg_value[i].value === "") {
            $('#usertable .change-table .change-table-form div form #' + reg_value[i].name).addClass('waring');
            Status = false;
        }
    });
    if (Status === true) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data === '666') {
                    ...
                    loadingOut();
                    $('#usertable .change-table .change-table-form div form div div #username').addClass('waring').attr('placeholder', '用户名已存在，请重新输入').val('');
                    window.location.href = "#username";
                }
                else if (data === '777') {
                    loadingOut();
                    $('#usertable .change-table .change-table-form div form div div #tel').addClass('waring').attr('placeholder', '手机号已存在，请重新输入').val('');
                    window.location.href = "#tel";
                }
                else if (data === '888') {
                    loadingOut();
                    $('#usertable .change-table .change-table-form div form div div #email').addClass('waring').attr('placeholder', '邮箱存在，请重新输入').val('');
