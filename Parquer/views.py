...
def systemMain(request, username, sid):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            # 判断是否有管理权限
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Parquer':
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            if user_obj in system_obj.admin.all() and system_obj.platform == 'Parquer':
                ...
                return render(request, 'parquer/parquerAnaly.html', locals())
            else:
                return redirect('/admin/' + request.session.get('USERNAME'))
        else:
            return redirect('/admin/' + request.session.get('USERNAME'))
...
def deviceDetail(request, username, sid, did):
    ...
    if is_login:
        if request.session.get('USERNAME') == username:
            ...
            if user_obj in device_obj.system.admin.all() and system_obj.platform == 'Parquer':
                ...
                elif device_map['Park'] == 0:
                    device_free = True
                elif device_map['Park'] == 1:
                    device_used = True
            ...
            if request.session.get('USERNAME') == username:
                ...
                if user_obj in device_obj.system.admin.all() and system_obj.platform == 'Parquer':
                    ...
                    if device_obj.data.filter(waring=1, model=0):
                        ...
                        new_map_data['status'] = 'used'
                    else:
                        new_map_data['status'] = 'free'
